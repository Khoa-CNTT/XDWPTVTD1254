                ...
                <div class="user-box dropdown">
                    <a class="d-flex align-items-center nav-link dropdown-toggle dropdown-toggle-nocaret" href="#"
                        ...
                        <div class="user-info ps-3">
                            <p class="user-name mb-0">{{ ten_co_quan }}</p>
                            <p class="designattion mb-0">Cơ Quan Y Tế</p>
                        </div>
                    </a>
...
export default {
    ...
    mounted() {
        this.ten_co_quan = localStorage.getItem('ten_co_quan')

    },
    methods: {
        dangXuat() {
            baseRequest
                .get('co-quan/dang-xuat')
                .then((res) => {
                    if (res.data.status) {
                        toaster.success('Thông báo<br>' + res.data.message);
                        window.localStorage.removeItem('chia_khoa_16');
                        window.localStorage.removeItem('ten_co_quan');
                        this.$router.push('/co-quan/dang-nhap');
                    } else {
